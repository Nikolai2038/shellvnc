...
#
# Usage: shellvnc_commands_must_be_installed <command...>
shellvnc_commands_must_be_installed() {
  local commands="$*"
  shellvnc_print_info_increase_prefix "Checking commands installation: ${c_highlight}${commands}${c_return}..." || return "$?"
  ...
  while [ "$#" -gt 0 ]; do
    ...
    if ! type "${command}" > /dev/null 2>&1; then
      ...
      elif [ "${_SHELLVNC_CURRENT_OS_NAME}" = "${_SHELLVNC_OS_NAME_MACOS}" ]; then
        shellvnc_print_error "Installing commands \"${c_highlight}${command}${c_return}\" are not implemented for \"${_SHELLVNC_OS_NAME_MACOS}\"!" || return "$?"
        return 1
      fi
      # ========================================
      ...
      if [ "${SHELLVNC_AUTO_INSTALL_PACKAGES}" = "1" ]; then
        shellvnc_print_info_increase_prefix "Installing \"${c_highlight}${command}${c_return}\" for ${_SHELLVNC_CURRENT_OS_NAME^} via command \"${c_highlight}${command_to_install}${c_return}\"..." || return "$?"
        eval "${command_to_install}" || return "$?"
        shellvnc_print_success_decrease_prefix "Installing \"${c_highlight}${command}${c_return}\" for ${_SHELLVNC_CURRENT_OS_NAME^} via command \"${c_highlight}${command_to_install}${c_return}\": success!" || return "$?"
        return 0
      else
        shellvnc_print_text "You can install \"${c_highlight}${command}${c_return}\" for ${_SHELLVNC_CURRENT_OS_NAME^} via command: \"${c_highlight}${command_to_install}${c_return}\"" || return "$?"
        shellvnc_print_error "Please install it manually or set \"${c_highlight}SHELLVNC_AUTO_INSTALL_PACKAGES=1${c_return}\" to install it automatically!" || return "$?"
        return 1
      fi
      # ========================================
    fi
  ...
  shellvnc_print_success_decrease_prefix "Checking commands installation: ${c_highlight}${commands}${c_return}: success!" || return "$?"
}
