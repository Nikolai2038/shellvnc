...
. "./shell/shellvnc_commands_must_be_installed.sh" || shellvnc_return_0_if_already_sourced || return "$?" 2> /dev/null || exit "$?"
...
shellvnc_install() {
  ...
  if [ "${type}" = "server" ] || [ "${type}" = "both" ]; then
    ...
    shellvnc_commands_must_be_installed vncviewer || return "$?"

    # TODO: Implement server installation
    # ...

    shellvnc_print_success_decrease_prefix "Server installation: success!" || return "$?"
  fi
  ...
  if [ "${type}" = "client" ] || [ "${type}" = "both" ]; then
    ...
    shellvnc_print_success_decrease_prefix "Client installation: success!" || return "$?"
  fi
