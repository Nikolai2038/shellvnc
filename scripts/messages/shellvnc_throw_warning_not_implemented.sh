...
. "./shellvnc_print_warning.sh" || shellvnc_return_0_if_already_sourced || return "$?" 2> /dev/null || exit "$?"
shellvnc_required_after_imports "${BASH_SOURCE[0]}" || return "$?" 2> /dev/null || exit "$?"

shellvnc_throw_warning_not_implemented() {
  if [ "$#" -ne 1 ]; then
    shellvnc_print_warning "Usage: ${c_highlight}${FUNCNAME[0]} \"\${LINENO}\"${c_return}" || return "$?"
    return 1
  fi

  local line_number="$1" && shift
  shellvnc_print_warning "Not implemented for \"${c_highlight}${_SHELLVNC_CURRENT_OS_NAME}${c_return}\" in file \"${c_highlight}${BASH_SOURCE[1]}${c_return}\" line \"${c_highlight}${line_number}${c_return}\"." || return "$?"
  return 0
}
